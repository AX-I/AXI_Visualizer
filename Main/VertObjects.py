...
class VertObject:    
    def __init__(self, *args, texture=None, texMode="default", gamma=True,
        ...
        if "emissive" in kwargs:
            ...
            ti = Image.open(ef).rotate(-90)
            if ti.mode == "RGB":
                ta = numpy.ceil(numpy.array(ti)[:,:,0] / 255).astype("bool")
            elif ti.mode == "RGBA":
                ta = numpy.array(ti)[:,:,0]
                ta = numpy.ceil(ta / 255).astype("bool")
            else:
                print(ti.mode)
            self.viewer.texAlphas.append(ta)
            self.viewer.vaNames[ef] = len(self.viewer.texAlphas)
            self.emissive = self.viewer.vaNames[ef]
        ...
        if texture in self.viewer.vtNames:
            ...
            s = ""
            if self.mip: s += "mip "
            if self.receiveShadow: s += "shadow "
            if self.useAlpha: s += "alpha "
            if self.reflection: s += "refl_" + self.reflection + " "
            s += " " + useShaders
            #if "cull" in x: s += "cull "
            #if "sky" in x: s += "sky "
...
class VertModel(VertObject):
    def __init__(self, *args, filename=None, size=1, mtlNum=0,
        ...
        if "plant" in self.mtlName or "Gold" in self.mtlName: self.subDiv = 0
