...
__kernel void drawTex(__global int *TO,
                      ...
                      const float ambLight, const float mipBias,
            ...
            if ((cy >= 0) && (cy < hF) && (ax >= 0) && (ax < wF)) {
              ...
              if (F[wF * cy + ax] > tz) {
                ...
                if ((sx >= shDist) && (sx < 2*wS-shDist) &&
                  ...
                  if (SD[2*wS * sy + sx] < depth) shadow += 5;
                ...
                if ((sx >= shDist) && (sx < 2*wS2-shDist) &&
                  ...
                  if (SD2[2*wS2 * sy + sx] < depth) shadow += sh2Pow*(1-sfy)*(1-sfx);
                  if (SD2[2*wS2 * (sy+1) + sx] < depth) shadow += sh2Pow*sfy*(1-sfx);
                  if (SD2[2*wS2 * sy + (sx+1)] < depth) shadow += sh2Pow*(1-sfy)*sfx;
                  if (SD2[2*wS2 * (sy+1) + (sx+1)] < depth) shadow += sh2Pow*sfy*sfx;
                }
