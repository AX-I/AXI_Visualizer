...
if (F[wF * cy + ax] > tz) {
    ...
    if (hit == 1) {
        ...
        Ro[wF * cy + ax] = (1-fr) * tsr + fr * (fade * ssr_out.x + (1-fade) * RR[rex]);
        Go[wF * cy + ax] = (1-fr) * tsg + fr * (fade * ssr_out.y + (1-fade) * RG[rex]);
        Bo[wF * cy + ax] = (1-fr) * tsb + fr * (fade * ssr_out.z + (1-fade) * RB[rex]);
        F[wF * cy + ax] = tz;
    }
    else {
        Ro[wF * cy + ax] = (1-fr) * tsr + fr * RR[rex];
        Go[wF * cy + ax] = (1-fr) * tsg + fr * RG[rex];
        Bo[wF * cy + ax] = (1-fr) * tsb + fr * RB[rex];
        F[wF * cy + ax] = tz;
    }
}
