// Perspective-correct triilinear texture drawing with smooth brightness
// 1 PCF shadow map and 1 opacity shadow map
...
__kernel void drawTex(__global int *TO,
                      ...
                      __global ushort *SD2, const int wS2, const float sScale2,
    ...
    int ytemp; int xtemp;
    float ut; float vt; float3 lt; float3 nt; float zt;
    float3 post;

    // bubble sort y1<y2<y3
    if (y1 > y2) {
      ytemp = y1; xtemp = x1; ut = u1; vt = v1; lt = l1; nt = n1; post = pos1; zt = z1;
      y1 = y2; x1 = x2; u1 = u2; v1 = v2; l1 = l2; n1 = n2; pos1 = pos2; z1 = z2;
      y2 = ytemp; x2 = xtemp; u2 = ut; v2 = vt; l2 = lt; n2 = nt; pos2 = post; z2 = zt;
    }
    if (y2 > y3) {
      ytemp = y2; xtemp = x2; ut = u2; vt = v2; lt = l2; nt = n2; post = pos2; zt = z2;
      y2 = y3; x2 = x3; u2 = u3; v2 = v3; l2 = l3; n2 = n3; pos2 = pos3; z2 = z3;
      y3 = ytemp; x3 = xtemp; u3 = ut; v3 = vt; l3 = lt; n3 = nt; pos3 = post; z3 = zt;
    }
    if (y1 > y2) {
      ytemp = y1; xtemp = x1; ut = u1; vt = v1; lt = l1; nt = n1; post = pos1; zt = z1;
      y1 = y2; x1 = x2; u1 = u2; v1 = v2; l1 = l2; n1 = n2; pos1 = pos2; z1 = z2;
        ...
        for (int ax = (int)cx2; ax <= (int)cx1; ax++) {
            if ((cy >= 0) && (cy < hF) && (ax >= 0) && (ax < wF)) {
              ...
              if (F[wF * cy + ax] > tz) {
                ...
                depth = dot(pos, SVd2);
                float sfx = (dot(pos, SVx2) * sScale2) + wS2;
                float sfy = (dot(pos, SVy2) * -sScale2) + wS2;
                sx = (int)sfx; sfx -= sx;
                sy = (int)sfy; sfy -= sy;
                if ((sx >= shDist) && (sx < 2*wS2-shDist) &&
                    (sy >= shDist) && (sy < 2*wS2-shDist)) {
                  shadow += 4*(1-sfy)*(1-sfx) * (1.f - exp((float)-SD2[2*wS2 * sy + sx] / 24.f));
                  shadow += 4*sfy*(1-sfx) * (1.f - exp((float)-SD2[2*wS2 * (sy+1) + sx] / 24.f));
                  shadow += 4*(1-sfy)*sfx * (1.f - exp((float)-SD2[2*wS2 * sy + (sx+1)] / 24.f));
                  shadow += 4*sfy*sfx * (1.f - exp((float)-SD2[2*wS2 * (sy+1) + (sx+1)] / 24.f));
                  
                  //if (SD2[2*wS2 * sy + sx] < depth) shadow += 3*(1-sfy)*(1-sfx);
                  //if (SD2[2*wS2 * (sy+1) + sx] < depth) shadow += 3*sfy*(1-sfx);
                  //if (SD2[2*wS2 * sy + (sx+1)] < depth) shadow += 3*(1-sfy)*sfx;
                  //if (SD2[2*wS2 * (sy+1) + (sx+1)] < depth) shadow += 3*sfy*sfx;
                }
                shadow = min(5.f, shadow);
                float light = shadow * ambLight + (5-shadow);
                light *= 0.2f;
                ...
                */
                ...
                outR += (ushort)((texi1*texi2*TR[tex] + texr1*texi2*TR[tex10] +
                                   texi1*texr2*TR[tex01] + texr1*texr2*TR[tex11])
                                   * (light * (dirCol.x + col.x) + (1-light) * col.x) * fmip);
                outG += (ushort)((texi1*texi2*TG[tex] + texr1*texi2*TG[tex10] +
                                   texi1*texr2*TG[tex01] + texr1*texr2*TG[tex11])
                                   * (light * (dirCol.y + col.y) + (1-light) * col.y) * fmip);
                outB += (ushort)((texi1*texi2*TB[tex] + texr1*texi2*TB[tex10] +
                                   texi1*texr2*TB[tex01] + texr1*texr2*TB[tex11])
                                   * (light * (dirCol.z + col.z) + (1-light) * col.z) * fmip);
                Ro[wF * cy + ax] = outR;
                Go[wF * cy + ax] = outG;
                Bo[wF * cy + ax] = outB;
