            ...
            if ((cy >= 0) && (cy < hF) && (ax >= 0) && (ax < wF)) {
              ...
              if (F[wF * cy + ax] > tz) {
                ...
                light *= 0.2f;
                    ...
                    float3 vcol = ((1.f-t)*cl2 + t*cl1);
                    float3 norm = fast_normalize(((1-t)*cn2 + t*cn1) * tz);
                    float3 dirCol;
                    if (dot(norm, LDir[0]) > 0.3f) dirCol = light * LInt[0] + (1.f-light)*vcol;
                    else dirCol = vcol;

                    Ro[wF * cy + ax] = convert_ushort((texi1*texi2*TR[tex] + texr1*texi2*TR[tex10] +
                                       texi1*texr2*TR[tex01] + texr1*texr2*TR[tex11]) * (light * dirCol.x));
                    Go[wF * cy + ax] = convert_ushort((texi1*texi2*TG[tex] + texr1*texi2*TG[tex10] +
                                       texi1*texr2*TG[tex01] + texr1*texr2*TG[tex11]) * (light * dirCol.y));
                    Bo[wF * cy + ax] = convert_ushort((texi1*texi2*TB[tex] + texr1*texi2*TB[tex10] +
                                       texi1*texr2*TB[tex01] + texr1*texr2*TB[tex11]) * (light * dirCol.z));
