__kernel void blurV(__global ushort *Ro, __global ushort *Go, __global ushort *Bo,
    ...
    for (int cy = h1; cy < min(h2, hF); cy++) {
        for (int cx = ci; cx < wF; cx += stepW) {
            float3 a = (float3)0;
            if ((cy > 8) && (cy < (hF-9))) {
              a += 0.3f*(float3)(Ro[wF * (cy-9) + cx], Go[wF * (cy-9) + cx], Bo[wF * (cy-9) + cx]);
              a += 0.3f*(float3)(Ro[wF * (cy-8) + cx], Go[wF * (cy-8) + cx], Bo[wF * (cy-8) + cx]);
              a += 0.3f*(float3)(Ro[wF * (cy-7) + cx], Go[wF * (cy-7) + cx], Bo[wF * (cy-7) + cx]);
