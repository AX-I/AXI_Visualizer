__kernel void blurV(__global ushort *Ro, __global ushort *Go, __global ushort *Bo,
    ...
    for (int cy = h1; cy < min(h2, hF-1); cy++) {
        for (int cx = ci; cx < wF; cx += stepW) {
            float3 a = (float3)0;
              ...
              Rt[wF * cy + cx] = (ushort)a.x;
              Gt[wF * cy + cx] = (ushort)a.y;
              Bt[wF * cy + cx] = (ushort)a.z;
        }
    }
    ...
    for (int cy = h1; cy < min(h2, hF); cy++) {
        for (int cx = ci; cx < wF; cx += stepW) {
          ...
          r2[wF * cy + cx] = r2[wF * cy + cx] + (texi1*texi2*Rt[tex] + texr1*texi2*Rt[tex10] + texi1*texr2*Rt[tex01] + texr1*texr2*Rt[tex11]);
          g2[wF * cy + cx] = g2[wF * cy + cx] + (texi1*texi2*Gt[tex] + texr1*texi2*Gt[tex10] + texi1*texr2*Gt[tex01] + texr1*texr2*Gt[tex11]);
          b2[wF * cy + cx] = b2[wF * cy + cx] + (texi1*texi2*Bt[tex] + texr1*texi2*Bt[tex10] + texi1*texr2*Bt[tex01] + texr1*texr2*Bt[tex11]);
        }
    }
}
