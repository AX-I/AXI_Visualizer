__kernel void blurH(__global ushort *r2, __global ushort *g2, __global ushort *b2,
    ...
    for (int cy = h1; cy < min(h2, hF); cy++) {
        for (int cx = ci; cx < wF; cx += stepW) {
            float3 a = (float3)0;
            if ((cx > 8) && (cx < (wF-9))) {
                a += 0.3f*(float3)(Ro[wF * cy + cx-9], Go[wF * cy + cx-9], Bo[wF * cy + cx-9]);
                a += 0.3f*(float3)(Ro[wF * cy + cx-8], Go[wF * cy + cx-8], Bo[wF * cy + cx-8]);
                a += 0.3f*(float3)(Ro[wF * cy + cx-7], Go[wF * cy + cx-7], Bo[wF * cy + cx-7]);
