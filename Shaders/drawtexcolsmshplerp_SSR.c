        ...
        for (int ax = (int)cx2; ax <= (int)cx1; ax++) {
            if ((cy >= 0) && (cy < hF) && (ax >= 0) && (ax < wF)) {
              ...
              if (F[wF * cy + ax] > tz) {
                ...
                float sd = REFL_DEPTH;
                for (int rn = 0;
                     (rn < REFL_LENGTH) && (sx >= 0) && (sx < wF) && (sy >= 0) && (sy < hF) && (sz > 0);
                     rn += vx) {
                    if ((F[wF * (int)sy + (int)sx] < 1.f/sz) &&
                        (F[wF * (int)sy + (int)sx] > 1.f/sz - sd) &&
                        (hit == 0)) {
                        Ro[wF * cy + ax] = Ro[wF * (int)sy + (int)sx];
                        Go[wF * cy + ax] = Go[wF * (int)sy + (int)sx];
                        Bo[wF * cy + ax] = Bo[wF * (int)sy + (int)sx];
                        F[wF * cy + ax] = tz;
                        hit = 1;
                    }
                    sx += slopex;
