...
class VertObject:    
    def __init__(self, *args, texture=None, texMode="default", gamma=True,
        ...
        if "alpha" in kwargs:
            ...
            if af in self.viewer.vaNames:
                ...
                if ti.mode == "1":
                    ta = numpy.array(ti).astype("bool")
                elif ti.mode == "RGB":
                    ta = (numpy.array(ti)[:,:,0] / 255).astype("bool")
                elif ti.mode == "RGBA":
                    ta = numpy.array(ti)[:,:,3]
        ...
        if texture in self.viewer.vtNames:
            ...
            if self.cgamma:
                ne.evaluate("ta * ta / 8.", out=ta) # Gamma correction
            else:
                ta *= 64
            ta *= self.texMul
    ...
    def created(self):
        ...
        if self.invertNorms:
            self.vertNorms *= -1
        self.wedgePos = numpy.mean(self.wedgePoints, axis=1)
                ...
                if line[0] == "#":
                    ...
                    if t[0] == "v":
                        ...
                        if activeMat:
                            ...
                            aw(numpy.array(c), -numpy.array(n), numpy.array(tx))
                            c = [self.points[int(s.split("/")[0]) - 1] for s in [t[1],t[3],t[4]]]
                            tx = [self.vts[int(s.split("/")[1]) - 1] for s in [t[1],t[3],t[4]]]
                            n = [self.vns[int(s.split("/")[2]) - 1] for s in [t[1],t[3],t[4]]]
                            aw(numpy.array(c), -numpy.array(n), numpy.array(tx))
...
class VertTerrain(VertObject):
    ...
    def create(self):
        ...
        for i in range(1, self.size[0]):
            ...
            p4 = self.heights[i][2:]
            v1 = numpy.stack((oo, p-p1, zz))
            v2 = numpy.stack((zz, p-p2, oo))
            v3 = numpy.stack((-oo, p-p3, zz))
            v4 = numpy.stack((zz, p-p4, -oo))
            n1 = numpy.cross(v1, v2, axis=0)
            n2 = numpy.cross(v2, v3, axis=0)
            n3 = numpy.cross(v3, v4, axis=0)
            n4 = numpy.cross(v4, v1, axis=0)
            nn = numpy.stack((n1, n2, n3, n4))
