...
__kernel void drawTex(__global int *TO,
                      ...
                      __global float *SD, const int wS, const int sScale,
    ...
    //if ((bx == 0) && (tx == 0)) printf("Pixel lenP: %d \n", lenP);
    if ((bx * BLOCK_SIZE + tx) < lenP) {
    int txd = TO[bx*BLOCK_SIZE + tx];
    //printf("TO[%d] = %d\n", bx*BLOCK_SIZE+tx, txd);
    int ci = txd * 3;
    ...
    float l4 = l1 + (float)(y2 - y1)/(float)(y3-y1) * (l3 - l1);
    ...
    float slopel1 = (l2-l1) / (float)(y2-y1);
    float slopel2 = (l4-l1) / (float)(y4-y1);
        ...
        for (int ax = (int)cx2; ax <= (int)cx1; ax++) {
            if ((cy >= 0) && (cy < hF) && (ax >= 0) && (ax < wF)) {
              ...
              if (F[wF * cy + ax] > tz) {
                ...
                float texr1 = ((1-t)*cu2 + t*cu1) * tz * (lenT-1);
                int tex1 = (int)texr1;
                texr1 -= tex1;
                tex1 = abs(tex1) % lenT;
                float texr2 = (lenT-1) * ((1-t)*cv2 + t*cv1) * tz;
                int tex2 = (int)texr2;
                texr2 -= tex2;
                tex2 = abs(tex2) % lenT;
                ...
                Ro[wF * cy + ax] = (ushort)((texi1*texi2*TR[tex] + texr1*texi2*TR[tex10] +
                                   texi1*texr2*TR[tex01] + texr1*texr2*TR[tex11]) * light);
                Go[wF * cy + ax] = (ushort)((texi1*texi2*TG[tex] + texr1*texi2*TG[tex10] +
                                   texi1*texr2*TG[tex01] + texr1*texr2*TG[tex11]) * light);
                Bo[wF * cy + ax] = (ushort)((texi1*texi2*TB[tex] + texr1*texi2*TB[tex10] +
                                   texi1*texr2*TB[tex01] + texr1*texr2*TB[tex11]) * light);
