// 1: Triangle setup, splits large triangles
    ...
    if (yes) {
      ...
      if ((max(dix.x, max(dix.y, dix.z)) - min(dix.x, min(dix.y, dix.z)) > 100) ||
        ...
        if (nextI < (BLOCK_SIZE - 4)) {
         ...
         ZZ[bInd*3+10] = dmz.s1;
         ZZ[bInd*3+11] = dmz.s2;
         TI[bx*BLOCK_SIZE + nextI] = bInd;
         TI[bx*BLOCK_SIZE + nextI+1] = bInd+1;
         TI[bx*BLOCK_SIZE + nextI+2] = bInd+2;
         TI[bx*BLOCK_SIZE + nextI+3] = bInd+3;
        }
      }
      else {
